...
从上述内容中，我们发现如果要应用到实际项目中，其实是有很多的琐碎的细节工作要完成的，为此我们引入中间件（middleware）来简化和隔离这些基础设施与业务逻辑之间的细节，让开发者能够关注在业务的开发上面。

在早期中间件的定义是，它是一种在操作系统上为应用软件提供服务的计算机软件。它既不是操作系统的一部分，也不是应用软件的一部分，它处于两者之间，让应用软件更好更方便的使用底层服务。如今中间件的含义借指了这种封装底层细节，为上层提供更方便服务的意义，并未限定在操作系统层面。这里我们使用的中间件就是，我们封装上文提及的所有 HTTP 请求细节处理的中间件。

1.可以采用 Connect 的设计，通过尾触发的方法实现。一个基本的中间件伪代码如下：
